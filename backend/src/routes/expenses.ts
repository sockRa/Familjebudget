...
router.get('/', validateQuery(YearMonthQuerySchema), (req: Request, res: Response) => {
    const { year_month } = req.query as unknown as { year_month?: number };
    res.json(db.getExpenses(year_month));
});
...
router.post('/:id/override', validateParams(IdParamSchema), validate(ExpenseUpdateSchema), (req: Request, res: Response) => {
    const { id: originalId } = req.params as unknown as { id: number };
...
router.post('/:id/hide/:yearMonth', validateParams(IdParamSchema.merge(YearMonthParamSchema)), (req: Request, res: Response) => {
    const { id: originalId, yearMonth } = req.params as unknown as { id: number, yearMonth: number };
